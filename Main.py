...
#Events
@client.event
async def on_voice_state_update(member, before, after): await Features.private_channel(member, before, after)


#Commands
@commands.command(name = "ping", description = "Replies with pong(bot's ping).")
async def ping(interaction): await Info.ping(interaction)

@commands.command(name = "clear", description = "Deletes set amount of messages from current channel.")
async def clear(interaction, ammount: int): await Admin.clear(interaction, ammount)

@commands.command(name = "mass_move", description = "Moves everyone from all channels to current channel or specified channel")
async def mass_move(interaction, channel: VoiceChannel): await Admin.mass_move(interaction, channel)

@commands.command(name = "asian", description = "Plays the selected Asian sound effect in the selected voice channel.")
async def sound(interaction, channel: VoiceChannel, sound: Literal['Stopid', 'Emotional Damage', 'Failure', 'I Will Send You To Jesus']): await Audio.engine(interaction, client, channel, sound, 'asian')
